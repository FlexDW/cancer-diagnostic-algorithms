...
cols <- c("red", "gold", "darkgrey", "green", "purple", "gold", "darkgrey")
names(cols) <- c("ridge", "GRR", "GRR2", "EN", "lasso", "GREN", "GREN2")
...
points(roc.GRR2, type='l', col=cols["GRR2"])
points(roc.GRR, type='l', col=cols["GRR"])
legtext <- paste(c("Ridge, ", "Grp Reg. Ridge (counts only), ", "Grp Reg. Ridge (+ tissue betas), "),
                 "AUC: ",
                 round(c(auc.ridge, auc.GRR2, auc.GRR), 2),
                 ", Sens: ",
                 round(c(sens.ridge, sens.GRR2, sens.GRR), 2),
                 sep="")
legend("bottomright", cex=0.8, lty=1,
       legend=legtext,
       col=cols[c("ridge", "GRR2", "GRR")],
...
legtext <- paste(c("Elastic Net, ", "Lasso, ", "Grp Reg. EN (counts only), ", "Grp Reg. EN (+ tissue betas), "),
                 "AUC: ",
                 round(c(auc.EN, auc.lasso, auc.GREN2, auc.GREN), 2),
                 ", Sens: ",
                 round(c(sens.EN, sens.lasso, sens.GREN2, sens.GREN), 2),
                 sep="")
legend("bottomright", cex=0.8, lty=1,
       legend=legtext,
       col=cols[c("EN", "lasso", "GREN2", "GREN")],
       title="AUC, Sensitivity at specificity=0.9")
dev.off()
