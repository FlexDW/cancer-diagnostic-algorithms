# Requires
# - PRAD: data list with iso_grro object loaded
...
# Lasso
vars.lasso <- whichSel(x=X, y=Y, nvars=nvars.PRAD, alpha=1, len=100)
...
vars.GREN <- PRAD$iso_grro$resEN$whichEN
...
# create plot
png(filename="Diagrams/compare_ROC_PRAD_isoforms.png", width=400, height=400)
plot(roc.ridge, type='l', col=cols["ridge"], 
     main="ROC comparison, tissue data")
points(roc.EN, type='l', col=cols["EN"])
points(roc.lasso, type='l', col=cols["lasso"])
points(roc.GREN, type='l', col=cols["GREN"])
legtext <- paste(c("Ridge (no selection), ", "Elastic Net (5 variables), ", "Lasso (5 variables), ", "Group Reg. EN (5 variables), "),
                 "AUC: ",
