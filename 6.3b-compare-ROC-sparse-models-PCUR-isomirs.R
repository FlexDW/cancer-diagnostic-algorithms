...
p.GRR <- cv.predict(x=cbind(1, PCUR$iso_grro0$XMw0), y=Y, alpha=0, lambda=cvo$lambda.min, foldid=cvSets, standardize=FALSE, intercept=FALSE, penalty.factor=rep(1, ncol(PCUR$iso_grro0$XMw0)))
...
legtext <- paste(c("Ridge (all), ", "Group-regularized Ridge (all), ", "Elastic Net (5 vars), ", "Group Reg. EN (5 vars), "),
                 "AUC: ",
                 round(c(auc.ridge, auc.GRR, auc.EN, auc.GREN), 3),
                 sep="")
legend("bottomright", cex=0.7, lty=1,
       legend=legtext,
       col=cols[c("ridge", "GRR", "EN", "GREN")],
       title="AUC")
dev.off()
