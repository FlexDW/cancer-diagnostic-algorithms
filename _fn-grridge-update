grridge <- function(highdimdata, response, partitions, unpenal = ~1, offset = NULL, 
                    ...
                    maxsel = 100, stepsel = 1, cvlmarg = 1, savepredobj = "all", 
                    dataunpen = NULL, ord = 1:length(partitions), comparelasso = FALSE, 
                    optllasso = NULL, cvllasso = T, compareEN = F, compareunpenal = FALSE, 
                    trace = FALSE, modus = 1) 
{
  ...
  if (length(monotone) != length(partitions)) {
    stop(paste("ERROR: length 'monotone' unequal to length 'partitions' "))
    return(NULL)
  }
  ...
  for (ncl in 1:nclass) {
    ...
    if (length(indexset) < nr) {
      warning(paste("Partition", ncl, "does not contain all row indices of the data"))
    }
    if (max(indexset) > nr | min(indexset) < 1) {
      stop(paste("ERROR: partition", ncl, "contains an invalid index, e.g. larger than number of data rows"))
      return(NULL)
    }
  }
  overlap <- c()
  ...
  for (ncl in 1:nclass) {
    ...
    if (length(indexset) > nfeatcl) {
      warning(paste("Grouping", ncl, "contains overlapping groups"))
  ...
  arguments <- list(partitions = partitions, unpenal = unpenal, 
                    ...
                    selection = selection, maxsel = maxsel, stepsel = stepsel, 
                    cvlmarg = cvlmarg, dataunpen = dataunpen, savepredobj = savepredobj, 
                    ord = ord, comparelasso = comparelasso, optllasso = optllasso, 
                    compareEN = compareEN, compareunpenal = compareunpenal, 
                    modus = modus)
  if (nr > 10000 & is.null(innfold)) 
    warning("NOTE: consider setting innfold=5 to save computing time")
  ...
  if (class(response) == "factor") {
    ...
    if (nlevel != 2) {
      stop("Response is not binary, so not suitable for two-class classification.")
      return(NULL)
    }
    else {
      model = "logistic"
    ...
    if (class(response) == "numeric" | class(response) == 
      ...
      if (class(response) == "Surv") {
        ...
        stop("Non-valid response. Should be binary, numeric or survival.")
        return(NULL)
      }
    }
  }
  if ((unpenal != ~0) & (unpenal != ~1)) {
    if (is.null(dataunpen)) {
      stop("If unpenal contains variables, data of the unpenalized variables should be specified in the data slot!")
      return(NULL)
    }
  }
  ...
  while (!converged & i <= niter) {
    ...
    while (cl <= nclass) {
      ...
      if (!convcl) {
        ...
        if (model == "linear") {
          ...
          if(trace) print(paste("Sigma^2 estimate:", VarRes))
        ...
        if (ellarg0 <= 0.5) {
          warning(paste("Partition", nmp0[cl], "NOT ITERATED"))
          conv[cl] <- TRUE
          cvln1 <- cvlnprev
          XMw0 <- XMw0prev
          pen <- penprev
          ...
          if(trace) print(paste("Shrink Factor coefficient matrix", 
                      lam))
          cfmmat <- coefmatfast
          ng <- nrow(cfmmat)
          dmax <- max(diag(cfmmat))
          ...
          if ((cvln1 - cvlnprev)/abs(cvlnprev) > 1/100 | 
            ...
            if (niter > 1) 
              if(trace) print(paste("Partition", nmp0[cl], "CONVERGED after", 
                          i, "iterations"))
    ...
    if (sum(conv) == nclass) {
      converged <- TRUE
      if (niter > 1) 
        if(trace) print(paste("All partitions CONVERGED after", 
                    i, "iterations"))
    }
    i <- i + 1
  }
  ...
  if (selection) {
    if(trace) print("Start posthoc variable selection")
    pmt <- proc.time()
    ...
    if(trace) print(paste("Number of selected markers by forward selection:", 
                nsel2))
    time3 <- proc.time() - pmt
    if(trace) print(paste("Computation time for feature selection by forward selection:", 
                time3[3]))
  }
  if (selection) 
  ...
  if (comparelasso) {
    ...
    if (is.null(optllasso)) {
      ...
      if(trace) print(paste("lambda1", optllasso))
