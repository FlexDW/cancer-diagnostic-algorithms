# File: 8.1b-compare-norm-strategies-PRAD-isomirs.R
# Requires
# - PRAD: data list with iso_grro object loaded (for selected variables)
...
cv_sets <- getCvSets(y=Y, nsets=5, seed=cvSeed, print=FALSE)

# selected variables
vars <- PRAD$iso_grro$resEN$whichEN

# normalized model
p_normed <- cv.predict(x=t(PRAD$isoDat[vars,]), y=Y, lambda=NULL, alpha=0, foldid=cv_sets)
...
# unnormalized model
p_unnormed <- cv.predict(x=t(PRAD$isoRaw[vars,]), y=Y, lambda=NULL, alpha=0, foldid=cv_sets)
...
PRAD$iso1normed <- t(normX(x=t(PRAD$isoRaw), n=1, norm_factors=PRAD$isoNF, foldid=cv_sets))
p_1normed <- cv.predict(x=t(PRAD$iso1normed[vars,]), y=Y, lambda=NULL, alpha=0, foldid=cv_sets)
...
PRAD$iso2normed <- t(normX(x=t(PRAD$isoRaw), n=2, norm_factors=PRAD$isoNF, foldid=cv_sets))
p_2normed <- cv.predict(x=t(PRAD$iso2normed[vars,]), y=Y, lambda=NULL, alpha=0, foldid=cv_sets)
...
PRAD$iso3normed <- t(normX(x=t(PRAD$isoRaw), n=3, norm_factors=PRAD$isoNF, foldid=cv_sets))
p_3normed <- cv.predict(x=t(PRAD$iso3normed[vars,]), y=Y, lambda=NULL, alpha=0, foldid=cv_sets)
...
png(filename="Diagrams/normalization_ROC_PRAD_isomirs.png", width=400, height=400)
plot(roc_unnormed, type='l', col=cols["Unnormalized"], 
     main="Normalization strategies on tissue data")
